 ...
 */
	...
	class ModeleLogement extends BaseDAO {
        ...
        public function filterLogements($filtre = "l_actif = true", $ordre = "evaluation DESC", $distance = "20", $latitude = "45.56", $longitude = "-73.57") {
            $sql = "SELECT *, (6371 * acos(cos(radians(" . $latitude . ")) * cos(radians(`latitude`)) * cos(radians(`longitude`) - radians(" . $longitude . ")) + sin(radians(" . $latitude . ") ) * sin(radians(`latitude`)))) AS distance FROM " . $this->lireNomTable() . " l JOIN al_disponibilite d ON l.id_logement = d.id_logement WHERE " . $filtre . " HAVING distance <= " . $distance . " ORDER BY " . $ordre;
			$resultat = $this->requete($sql);
			return $resultat->fetchAll(PDO::FETCH_CLASS|PDO::FETCH_PROPS_LATE, "Logement");
    	}        
		...
		public function sauvegarderLogement(Logement $logement) {
			/*if ($logement->lireId() && $this->lire($logement->lireId())->fetch()) {
				// update
				$sql = "UPDATE " . $this->lireNomTable() . " SET no_civique=?, apt=?, rue=?, ville=?, province=?, pays=?, code_postal=?, latitude=?, longitude=?, id_type_logement=?, nb_interieur=?, premiere_photo=?, prix=?, evaluation=?, description=?, courriel=?, nb_personnes=?, nb_chambres=?, nb_lits=?, nb_salle_de_bain=?, nb_demi_salle_de_bain=?, frais_nettoyage=?, est_staionnement=?, est_wifi=?, est_cuisine=?, est_tv=?, est_fer_a_repasser=?, est_cintres=?, est_seche_cheveux=?, est_climatise=?, est_laveuse=?, est_secheuse=?, est_chauffage=?, l_banni=?, l_date_banni=?, l_commentaire_banni=? WHERE " . $this->lireClePrimaire() . "=?";
                $donnees = array($logement->lireNoCivique(), $logement->lireApt(), $logement->lireRue(), $logement->lireVille(), $logement->lireProvince(), $logement->lirePays(), $logement->lireCodePostal(), $logement->lireLatitude(), $logement->lireLongitude(), $logement->lireCourriel(), $logement->lireIdTypeLogement(), $logement->lireNbInterieur(), $logement->lirePremierePhoto(), $logement->lirePrix(), $logement->lireEvaluation(), $logement->lireDescription(), $logement->lireNbPersonnes(), $logement->lireNbChambres(), $logement->lireNbLits(), $logement->lireNbSalleDeBain(), $logement->lireNbDemiSalleDeBain(), $logement->lireFraisNettoyage(), $logement->lireEstStaionnement(), $logement->lireEstWifi(), $logement->lireEstCuisine(), $logement->lireEstTv(), $logement->lireEstFerARepasser(), $logement->lireEstCintres(), $logement->lireEstSecheCheveux(), $logement->lireEstClimatise(), $logement->lireEstLaveuse(), $logement->lireEstSecheuse(), $logement->lireEstChauffage(), $logement->lireLBanni(), $logement->lireLDateBanni(), $logement->lireLCommentaireBanni(), $logement->lireIdLogement());
			}
			else {*/
				// insert

                $sql = "INSERT INTO " . $this->lireNomTable() . "(
					no_civique, apt, rue, ville, province, 
					pays, code_postal, latitude, longitude, id_type_logement, 
					prix, evaluation, description, courriel, nb_personnes, 
					nb_chambres, nb_lits, nb_salle_de_bain, nb_demi_salle_de_bain, 
					est_stationnement, est_wifi, est_cuisine, est_tv, est_fer_a_repasser, 
					est_cintres, est_seche_cheveux, est_climatise, est_laveuse, est_secheuse, 
				...
				$donnees = array(
					...
					$logement->lirePrix(), $logement->lireEvaluation(), $logement->lireDescription(), $logement->lireCourriel(), $logement->lireNbPersonnes(),
